...
var CE = (function(ce_){
    ...
    function search(store, index, qstring, deep){
        ...
        if(qstring.length > 0)
        {
            ...
            // TIM: 1/12/2017 force a shallow search to increase speed by using range
            deep = false;
            ...
            request.onsuccess = function(e){
                ...
                if(result)
                {
                    ...
                    for(var i = 0; i < result.length; i++)
                    {
						...
						/*
						 if(result[i].name_lc)
						 {
						 if(result[i].name_lc.search(qstring[0]) >= 0)
						 list.push(result[i]);
						 }
						 if(result[i].sku)
						 {
						 if(result[i].sku.search(qstring[0]) >= 0)
						 list.push(result[i]);
						 }
						 */
                    }
				...
				/*
				 ...
				 {
				 console.log('finished... here is the list:');
				 console.log(list);
				 def.resolve(list);
				 }*/
    ...
    function getAllFrom(store){
        ...
        {
            var result = e.target.result;
            var index = e.target.source.keyPath;
            if(result)
            {
                for(var i = 0; i < result.length; i++)
                {
    ...
    function putAllInto(list, store){
        ...
        console.log('attempting to put all: ');
        for(var i in list)
            os.put(list[i]);
        console.log('done putting into db');
